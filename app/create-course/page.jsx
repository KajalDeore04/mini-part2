...
const CreateCourse = () => {
  ...
  const isFormComplete = () => {
    const requiredFields = ['category', 'topic', 'level', 'duration', 'displayVideo', 'noOfChapter'];
    return requiredFields.every(field => !!userCourseInput?.[field]);
  };

  const checkProgress = () => {
    let filledCount = 0;
    const fields = ['category', 'topic', 'level', 'duration', 'displayVideo', 'noOfChapter'];
  ...
  const GenerateCourseLayout = async () => {
    setLoading(true);
    const prompt = `Generate a Course tutorial on: Category:${userCourseInput?.category}, 
  ...
  const getDifficultyColor = (levelId) => {
    return difficultyLevels.find(l => l.id === levelId)?.color || "text-white";
  };
        ...
        >
          <Card className="relative bg-gray-900 border border-gray-800 shadow-2xl overflow-hidden rounded-xl backdrop-blur-sm bg-opacity-80">
            ...
            <div className="p-8 space-y-8 mt-1">
              ...
              {/* Category Selection */}
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-3">Select Category</label>
                  ...
                  {categoryOptions.map((category) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        `}>
                          <motion.div 
                            className={`${isActive 
                              ? category.textColor
                              : 'text-white'}`}
                            animate={animateIcon === category.id ? 
                        ...
                        <div className={`p-2 text-center text-white ${isActive ? category.activeBg : 'bg-gray-900'}`}>
              ...
              >
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Course Topic</label>
                  <Input
              ...
              >
                      ...
                      return (
                        ...
                        `}>
                          <RadioGroupItem 
                            value={level.id} 
                            id={`level-${level.id}`} 
                            className={`border-gray-500 ${isActive ? level.color : 'text-white'}`} 
                          />
                          <Label 
                            htmlFor={`level-${level.id}`} 
                            className={`cursor-pointer text-sm mt-1 ${isActive ? level.color : 'text-white'}`}
                          >
                            {level.id}
                          </Label>
                        </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-3">
                      <span className="text-xs text-gray-400">
                        {userCourseInput?.displayVideo === "Yes" ? "Enabled" : "Disabled"}
                      </span>
                      <Switch 
                        id="video-toggle"
                        checked={userCourseInput?.displayVideo === "Yes"}
                        onCheckedChange={(checked) => {
                          handleInputChange('displayVideo', checked ? "Yes" : "No");
                        }}
                ...
                <div className="bg-gray-850 rounded-lg p-4 border border-gray-800">
                  <label className="block text-sm font-medium text-gray-400 mb-3">Number of Chapters</label>
                  <div className="flex items-center justify-between">
                    <Button 
                      variant="outline" 
                      size="icon"
                      onClick={() => handleChapterChange(false)}
                      disabled={!userCourseInput?.noOfChapter || parseInt(userCourseInput.noOfChapter) <= 1}
                    ...
                    <Button 
                      ...
                      disabled={userCourseInput?.noOfChapter && parseInt(userCourseInput.noOfChapter) >= 20}
                      className="h-10 w-10 rounded-xl bg-gray-800 border-gray-700 text-white hover:bg-gray-700"
                    >
  ...
  whileHover={isFormComplete() ? { scale: 1.05, boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.1)" } : undefined}
  whileTap={isFormComplete() ? { scale: 0.98 } : undefined}
>
  <Sparkles className={`h-5 w-5 ${checkProgress() >= 100 ? 'text-white' : ''}`} />
        ...
        {(userCourseInput?.category || userCourseInput?.topic) && (
          ...
          >
            <Card className="p-6 bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-xl">
              <h3 className="text-lg font-medium text-gray-400 mb-4 border-b border-gray-800 pb-2">Course Preview</h3>
              ...
              <div className="flex flex-col md:flex-row gap-4 items-center">
                {userCourseInput?.category && (
                  ...
                  >
                    <div className={getCategoryDetails(userCourseInput.category).textColor}>
                ...
                <div className="flex-grow">
                  ...
                  >
                    ...
                    {userCourseInput?.description && (
                      <p className="text-sm text-gray-400 mt-1 line-clamp-2">{userCourseInput.description}</p>
                    )}
                  </motion.div>
                  ...
                  <div className="flex flex-wrap gap-4 mt-3">
                    {userCourseInput?.level && (
                      <motion.div 
                        className={`flex items-center px-3 py-1 rounded-full bg-gray-800 text-xs ${getDifficultyColor(userCourseInput.level)}`}
                    ...
                    {userCourseInput?.displayVideo && (
                      <motion.div 
                        className={`flex items-center px-3 py-1 rounded-full bg-gray-800 text-xs ${userCourseInput.displayVideo === "Yes" ? "text-green-400" : "text-gray-400"}`}
                        initial={{ opacity: 0, y: 5 }}
                        animate={{ opacity: 1, y: 0 }}
                ...
                {isFormComplete() && (
                  <motion.div 
                    ...
                    animate={{ scale: 1, opacity: 1 }}
                    transition={{ type: "spring", stiffness: 500, damping: 30, delay: 0.9 }}
                  >
        ...
        <AlertDialogTitle>Course Generation Status</AlertDialogTitle>
          ...
          <div className="py-8 flex flex-col items-center justify-center gap-5">
            ...
            <div className="text-center">
              <p className="text-lg font-medium text-white mb-1">Building Your Course</p>
